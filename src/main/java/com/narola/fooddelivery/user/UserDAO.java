...
public class UserDAO {
	...
	public User findUser(String username, String password) throws DatabaseException{
		...
		try {
			...
			if (resultSet.next()) {
				...
				if(user.getAdmin()==3)
					user.setRestaurantId(restDao.getRestaurantFromUserId(resultSet.getInt("userId")).getRestaurantId());
			}
		} catch (SQLException e) {
	...
	public User findByUserId(int id) throws DatabaseException{
		...
		try {
			...
			if(resultSet.next()) {
				...
				user.setPassword(resultSet.getString("password"));
				user.setAdmin(resultSet.getInt("usertype"));
				user.setRestaurantId(restDao.getRestaurantFromUserId(id).getRestaurantId());
			}
		} catch (SQLException e) {
